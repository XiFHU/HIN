...
Update notes for App2.3 no-upload sources
-----------------------------------------
- Road Network now includes a no-upload OSM option. Enter a study-area place name, download OSM roads, map OSM highway tags to functional classes, and generate FromMile/ToMile.
- Crash Data now includes a no-upload FARS option using the official NHTSA CrashAPI. FARS contains fatal crashes only.
- Interactive Folium basemaps were changed from colorful OpenStreetMap tiles to the light-gray CartoDB Positron OSM-based basemap.
- Sliding-window method settings were not changed for the comparison request. See SLIDING_WINDOW_METHOD_COMPARISON.md.
