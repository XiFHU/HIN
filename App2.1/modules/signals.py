...
def assign_corridor_ids_to_signals(
    ...
    This version keeps all nearby signals in the output.

    Signals on routes with at least min_signals receive CorridorID.
    Signals not on valid corridors remain in the table, but CorridorID is null.

    This allows the map to show one complete signal layer while still using
    only valid corridor signals to build corridors.
    """
    ...
    roads_named["Route_Normalized"] = (
        roads_named[road_name_col]
        .apply(normalize_road_name)
    )

    signals_points = signals_m.copy()
    ...
    signals_with_road = signals_with_road.merge(
        signals_points[
            [
                "SignalID",
                "geometry"
    ...
    signals_with_road["DistRoad"] = (
        signals_with_road.geometry.distance(
            roads_named.geometry.union_all()
        )
    )
