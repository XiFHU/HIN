...
def render_corridors_step(st_folium, workflow_context, spatial_unit=None):
    ...
    if selected_roads is not None and signals_clean is not None:
        ...
        if use_corridor_road_filter:
            ...
            road_class_cols = [
                c for c in corridor_roads.columns
                if c != "geometry"
            ]
        ...
        if corridor_roads.empty:
            ...
            if st.button(
                ...
                with st.spinner(
                    ...
                    if route_col not in corridor_roads.columns:
                        st.error(
                            f"Route column '{route_col}' is not in the corridor road layer."
                        )
                        st.stop()

                    signal_assignment_roads = (
                        base_roads.copy()
                    )
