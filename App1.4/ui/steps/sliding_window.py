...
def render_sliding_window_step(st_folium, workflow_context, spatial_unit=None):
    ...
    if selected_roads is None:
        ...
        if "section7_results" in st.session_state:
            ...
            with c2:
                st.download_button(
                    "Download Sliding Window Risk Segments GeoJSON",
            ...
            st.caption(
                "GeoJSON export is limited to original crash density and sliding-window risk segments."
            )
            ...
            st.subheader("Final HIN Risk Comparison Map")

            st.caption(
                "Use the small layer control in the map top-right corner to turn Original Crash Density, HIN Risk Score, Risk Corridors, Crashes, Signals, Roads, or Corridors on/off."
            )
