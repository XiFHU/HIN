...
def _make_segment_comparison_map(
    ...
    """Create the final Segment comparison map without adding a duplicate Folium LayerControl."""
    ...
    if "Current Spatial Units / Crash Density" in selected_layers:
        ...
        if spatial_units is not None and not spatial_units.empty:
            ...
            values = pd.to_numeric(
                spatial_units.get("CrashDensity", 0),
                errors="coerce"
            ).fillna(0)
    ...
    if "Risk Segments" in selected_layers:
...
def render_sliding_window_step(st_folium, workflow_context, spatial_unit=None):
    ...
    if selected_roads is None:
        ...
        with st.expander("7.1 Analysis Settings", expanded=True):
        ...
        if "section7_results" in st.session_state:
            ...
            # -----------------------------
            # Risk Segment Table / Downloads
            # -----------------------------

            st.subheader("Risk Segment Table")
            ...
            with c2:
                st.download_button(
                    "Download Risk Segments GeoJSON",
                    data=gdf_to_geojson_bytes(
                        risk_segments_clean
                    ),
                    file_name="section7_risk_segments.geojson",
            ...
            # -----------------------------
            # Risk Corridor Table / Downloads
            # -----------------------------

            st.subheader("Risk Corridor Table")
            ...
            with c2:
                st.download_button(
                    "Download Risk Corridors GeoJSON",
                    data=gdf_to_geojson_bytes(
                        risk_corridors_clean
                    ),
                    file_name="section7_risk_corridors.geojson",
                    mime="application/geo+json",
                    key="section7_download_corridors_geojson"
                )
            ...
            st.subheader("Final Segment Comparison Map")

            st.caption(
                "Compare the original crash-density pattern against the HIN risk result. "
                "Crashes are optional because they can make the map crowded."
            )
            ...
            comparison_map = _make_segment_comparison_map(
                ...
                roads=st.session_state.get("roads_map_display", selected_roads),
